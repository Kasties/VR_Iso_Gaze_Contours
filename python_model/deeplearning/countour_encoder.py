...
def main(args):
    ...
    picklefile = '/home/mar/ai/VR_Iso_Gaze_Contours/python_model/dataset9/datasetreal/predictions/parameters.pkl'
    #load data
    ...
    #print(f"Value: {contour_G_labels}, Type: {type(contour_G_labels)}")

    print(contour_G_labels.shape)
    contour_G_labels = np.array([np.array(i, dtype=float) for i in contour_G_labels.to_list()])
    # Create a boolean mask for non-empty lists
    non_empty_mask = np.array([len(sublist) > 0 for sublist in contour_G_labels])
    contour_G_labels = contour_G_labels[non_empty_mask]
    b = []
    # Use the mask to filter out the empty lists
    for i in range(len(contour_G_labels)):
       # print(contour_G_labels[i])
        b.append(contour_G_labels[i].tolist())
        
        
    for i in contour_G_labels:
      #  print(f"Value: {i}, Type: {type(i)}")
        pass
    print(np.array(b).shape)
    contour_G_labels = np.array(b)
    contour_G_labels = contour_G_labels.reshape((-1,42))
    print(contour_G_labels.shape)
    quit()
