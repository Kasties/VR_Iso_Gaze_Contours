...
def main(args):
  ...
  path = args.path
  outjsonpath = os.path.join(path,"countour_model_light.json")
  checkpoint_filepath = os.path.join(path,"checkpointSupervised_light")
  picklefile = os.path.join(path,"predictions\\parameters.pkl")
  ...
  contour_features.pop('imagePCenter')
  contour_features.pop('contoursG')
  contour_features.pop('contoursGCenter')
  contour_features.pop('imageGCenter')
  ...
  x = layers.Dense(512,activation=relu)(input_dist)
  ...
  # encoder
  contour_model = Model(input_dist, [out1, out2])
