...
from  deeplearning import countour_encoder_prediction
#gui
from matplotlib.widgets import Slider, Button, RadioButtons

VelocityAngle = None
VelocityAngleRange = None
VelocityMagnitude = None
gaussian_contour = []
gaussian_contour_center=[]
ContourPercentile = None
fig2 = None
axes =None
saveIMG = False
p2 = None
...
contour_model = countour_encoder_prediction.loadModel(datasetpath)

def update(val):
    global gaussian_contour,VelocityAngle,VelocityMagnitude,ContourPercentile,gaussian_contour_center

    # recalculate
    #conditions = recalculateConditions()

    # inference
    prediction_contour, prediction_contour_center, prediction_image_center,gaussian_contour, gaussian_contour_center, gaussian_image_center = countour_encoder_prediction.predict(VelocityAngle.val,VelocityMagnitude.val,ContourPercentile.val,contour_model)
...
def displayConditions(df,conditions):
    global fig2,axes,VelocityAngle,VelocityAngleRange,saveIMG,p2,gaussian_contour,VelocityMagnitude,ContourPercentile,gaussian_contour_center
    ...
    if(fig2 is None):
        ...
        VelocityAngle = Slider(axampVelocityAngle, 'Angle', -180, 180, valinit=0)

        # axampVelocityAngleRange= plt.axes([0.2, 0.04, 0.65, 0.02], facecolor=axcolor)
        ...
        VelocityMagnitude.on_changed(update)
        ContourPercentile.on_changed(update)
    ...
    for i,condition in enumerate(conditions):
        ...
        axe.clear()
        axe.vlines(0,-50,50, colors='black')
        axe.hlines(0,-50,50,colors='black')
        ...
        if gaussian_contour_center != []:
            axe.scatter(gaussian_contour_center[:,0],gaussian_contour_center[:,1],c='green')
        if gaussian_contour != []:
